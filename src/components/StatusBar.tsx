import React from 'react';
import { Activity, Move, Layers, Maximize, Cpu } from 'lucide-react';

interface StatusBarProps {
  width: number;
  height: number;
  zoom: number;
  cursorPos: { x: number; y: number; pressure: number };
  activeLayerName: string;
  activeTool: string;
}

export const StatusBar: React.FC<StatusBarProps> = ({
  width,
  height,
  zoom,
  cursorPos,
  activeLayerName,
  activeTool,
}) => {
  return (
    <footer className="h-6 bg-slate-900 border-t border-slate-800 flex items-center justify-between px-3 text-[11px] text-slate-400 select-none z-30">
      {/* Left: Canvas Dimensions & Zoom */}
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-1" title="Canvas Dimensions">
          <Maximize className="w-3 h-3 text-slate-500" />
          <span className="font-mono text-slate-300">
            {width} × {height} px
          </span>
        </div>

        <div className="flex items-center space-x-1" title="Zoom Level">
          <span className="text-slate-500 font-medium">Zoom:</span>
          <span className="font-mono text-sky-400 font-semibold">
            {Math.round(zoom * 100)}%
          </span>
        </div>
      </div>

      {/* Middle: Active Layer & Active Tool */}
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-1.5" title="Active Layer">
          <Layers className="w-3 h-3 text-sky-400" />
          <span className="text-slate-300 font-medium truncate max-w-[120px]">
            {activeLayerName || 'No Layer'}
          </span>
        </div>

        <div className="flex items-center space-x-1">
          <span className="text-slate-500">Tool:</span>
          <span className="text-slate-200 capitalize font-medium">{activeTool}</span>
        </div>
      </div>

      {/* Right: Cursor Coordinates & Pressure Gauge */}
      <div className="flex items-center space-x-4">
        <div className="flex items-center space-x-1 font-mono" title="Cursor Coordinates">
          <Move className="w-3 h-3 text-slate-500" />
          <span>
            X: <strong className="text-slate-200">{cursorPos.x}</strong> Y:{' '}
            <strong className="text-slate-200">{cursorPos.y}</strong>
          </span>
        </div>

        <div
          className="flex items-center space-x-1.5 bg-slate-800/80 px-2 py-0.5 rounded border border-slate-750"
          title="Pointer/Stylus Pressure Sensitivity"
        >
          <Activity className="w-3 h-3 text-indigo-400" />
          <span className="text-slate-500">Pressure:</span>
          <span className="font-mono text-indigo-300 font-bold">
            {Math.round(cursorPos.pressure * 100)}%
          </span>
        </div>
      </div>
    </footer>
  );
};
